...
(function () {
    ...
    const USER_AGENT = 'SponsorBlockMobileUserscript/1.0.0';
    ...
    const Config = {
        enabled: storageGet('enabled', true),
        serverAddress: storageGet('serverAddress', DEFAULT_SERVER),
        categoryActions: Object.assign(defaultCategoryActions(), storageGet('categoryActions', {})),
        userID: storageGet('userID', null),
        minDuration: storageGet('minDuration', 0),
        stats: storageGet('stats', { segmentsSkipped: 0, secondsSaved: 0 }),
        showProgressBarSegments: storageGet('showProgressBarSegments', true),
    ...
    const segmentCache = new Map(); // videoID -> { segments, fetchedAt }
    ...
    const PlaybackState = {
        ...
        pendingSubmission: [], // {start, end, category, actionType}
        toastEl: null,
        toastTimer: null,
        manualBtnEl: null,
        manualBtnUUID: null,
    ...
    function openSettings() {
            ...
            const toggle = h('div', { class: 'sbm-seg-toggle' }, options.map(([val, text]) =>
                h('button', {
                    ...
                    onclick: (e) => {
                        ...
                        for (const b of toggle.children) b.classList.remove('active');
        ...
        settingsScreen = h('div', { class: 'sbm-overlay-screen', onclick: (e) => { if (e.target === settingsScreen) closeSettings(); } }, [sheet]);
    ...
    function renderSubmitSheet() {
        const list = h('div', {}, PlaybackState.pendingSubmission.map((seg, idx) => {
            const select = h('select', {
                onchange: (e) => { seg.category = e.target.value; },
        ...
        if (submitScreen) {
            ...
            submitScreen = h('div', { class: 'sbm-overlay-screen', onclick: (e) => { if (e.target === submitScreen) closeSubmit(); } }, [sheet]);
            document.body.appendChild(submitScreen);
        }
    }

    /* ------------------------------------------------------------------ *
     *  Floating action buttons (settings / submit)
     * ------------------------------------------------------------------ */
